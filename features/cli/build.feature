Feature: build 'apps' with CLI
  ...
  Scenario: io.openshift.build.commit.ref displays correctly in build reference on imagestreamtag if building from git branch reference
    Given I have a project
    When I run the :new_app client command with:
      | app_repo | quay.io/pdsilva1/ruby-27:multiarch~https://github.com/openshift/ruby-hello-world#config |
    Then the step should succeed
  ...
  Scenario: Allow using a configmap as an input to a docker build
    ...
    When I run the :new_build client command with:
      | app_repo         | quay.io/pdsilva1/ruby-27:multiarch~https://github.com/openshift/ruby-hello-world |

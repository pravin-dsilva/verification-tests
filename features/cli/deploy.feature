Feature: deployment related features
  ...
  Scenario: Rollback via CLI when previous version failed
    ...
    When I run the :create_deploymentconfig client command with:
      | image | quay.io/pdsilva1/hello-openshift@sha256:1c3ddda53bff4c1221e879fd392990d4b7445748f0f95ee56bcef99da6177ae2 |
      | name  | mydc                  |
    Then the step should succeed
  ...
  Scenario: A/B Deployment
    ...
    When I run the :new_app client command with:
      | docker_image | quay.io/pdsilva1/deployment-example@sha256:2de5e0f06112c0bff497ed53049f9486b32bd36c1f4176c0ffd1251b71115adc |
      | name         | ab-example-a                                                                                                     |
      | l            | ab-example=true                                                                                                  |
      | env          | SUBTITLE=shardA                                                                                                  |
    Then the step should succeed
  ...
  Scenario: Blue-Green Deployment
    Given I have a project
    When I run the :new_app client command with:
      | docker_image | quay.io/pdsilva1/deployment-example:v1-multiarch |
      | name         | bluegreen-example-old                       |
    Then the step should succeed
    When I run the :new_app client command with:
      | docker_image | quay.io/pdsilva1/deployment-example:v2-multiarch |
  ...
  Scenario: Scale up when deployment running
    Given I have a project
    When I run the :create_deploymentconfig client command with:
      | image | quay.io/pdsilva1/deployment-example@sha256:2de5e0f06112c0bff497ed53049f9486b32bd36c1f4176c0ffd1251b71115adc |
